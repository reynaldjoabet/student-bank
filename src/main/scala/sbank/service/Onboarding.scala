...
/**
  ...
  *
  * Critical privacy property: the raw SSN passes through this service exactly once (in
  * `submitKyc`). We compute `SsnHash` (HMAC-SHA256 with the server pepper) and `SsnLast4` (display
  * only), then discard the raw value. The DB never receives plaintext.
  */
...
object Onboarding {
  ...
  /**
    * HMAC-SHA256 hex digest. The pepper is server-side only — without it the hash is useless for
    * SSN lookup, even with a leaked database.
    */
  ...
  /**
    * Convenience: derive a deterministic pepper from an env-var seed using SHA-256. Production:
    * pull the real pepper from KMS / Key Vault.
    */
